...
description: Try the RAIS protocol live in your browser. No API key needed. A real streaming endpoint running inside this docs site.
---
...
A live RAIS-compliant streaming endpoint running inside this docs site. No API key, no setup, no backend required.

<Callout type="info">
  This demo streams real RAIS events — `text`, `done` — over SSE from a Next.js API route at `/api/demo`. Open your browser DevTools → Network tab → filter by `demo` to watch the raw stream.
</Callout>
